...
class Cortex(object):
    ...
    def __init__(self, client_id_file_path):
        ...
        self.id_sequence = 0
        self.session_ids = []
        self.headset2session_id = {}
        self.session_id2headset = {}
    ...
    async def create_session(self, activate, headset_id=None):
        ...
        new_session_id = resp['result']['id']
        self.session_ids.append(new_session_id)
        self.headset2session_id[headset_id] = new_session_id
        self.session_id2headset[new_session_id] = headset_id
        logger.debug(f"{__name__} resp:\n{resp}")
    ...
    # async def create_record(self, titles=None):
    async def create_record(self, titles):
        resps = []
        for session_id, title in zip(self.session_ids, titles):
            params = {'cortexToken': self.auth_token,
                      # 'session': self.session_id,
                      'session': session_id,
                      'title': title}
